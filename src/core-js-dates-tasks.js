...
function getTime(date) {
  const h = addZeroToTime(date.getHours());
  const m = addZeroToTime(date.getMinutes());
  const s = addZeroToTime(date.getSeconds());
...
const getUTCTime = (date, callback) => {
  const utcHours = date.getUTCHours();
  const hours = utcHours > 12 ? utcHours - 12 : utcHours;
  return {
    seconds: callback(date.getUTCSeconds()),
    minutes: callback(date.getUTCMinutes()),
    hours,
  };
};
...
function formatDate(isoDate) {
  ...
  const meridiem = getMeridiem(date.getUTCHours());
  const { seconds, minutes, hours } = getUTCTime(date, addZeroToTime);
  const { day, month, year } = getUTCDate(date);
 ...
 */
function getNextFridayThe13th(date = new Date()) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  let start = getTimeFromString(period.start);
  const end = getTimeFromString(period.end);
  const day = 864e5;
  ...
  for (; start < end; ) {
    for (let i = 0; i < countWorkDays; i += 1) {
      ...
      res.push(transformMsToDate(start));
    }
    start += offDays;
  }
  return res;
